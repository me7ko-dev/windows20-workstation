...
- **ИИ чатът е Genesis** (`Ctrl+Shift+I` или `Ctrl+Space I`) — твоят агент
  [genesis-agent](https://github.com/me7ko-dev/genesis-agent), през неговото
  OpenAI-съвместимо API на `127.0.0.1:8100`. Genesis е **отделен процес**:
  станцията го пуска, когато тръгне, но той остава да работи и след нея.
  **„Инсталирай / обнови Genesis“** (лентата, Настройки или бутонът в дока)
  го слага в терминал от клона с ъпгрейда (`claude/token-upgrade-ipe4yg`):
  с неговия `install_windows.ps1` и с `fastapi` и `uvicorn` за API-то.
  „Genesis в отделен прозорец“ отваря неговия чат извън станцията. Ако го няма,
  отговаря същата безплатна услуга като гласа (Groq, Gemini, OpenRouter,
  Grok или Ollama) и под отговора пише защо. Genesis е и в дока, като агент
  в терминал. Отговорът тече, докато се пише, и се
  спира с ■. Команда в блок с код има два бутона: **Копирай** и
  **› В терминала**, който я написва в последния ползван терминал, без да
  натиска Enter. Разговорът се пази със станцията. В лентата всеки текст има
  и ред „Питай в ИИ чата“.
- **Светла тема** (`Ctrl+Alt+L`, или в Настройки: тъмна / светла / като
  Windows). Прозорците, лентата и картите стават светли, терминалите минават
  на светла палитра, а бутоните на Windows горе вдясно се пребоядисват.
- **Свои клавиши** в `keybindings.json`. „Промени клавишите…“ (в F1, в
  Настройки или в лентата) го създава с всички клавиши и ги отваря в
  редактора. Сменяш, запазваш и новите важат веднага, без рестарт. Сгрешен
  клавиш или две действия на един клавиш се казват в съобщение. Разрешени са
  `//` коментари и излишна запетая.
- **Глобални клавиши от Windows.** `Ctrl+Alt+W` показва станцията отвсякъде,
  дори когато е скрита; ако вече е отпред, я скрива. `Ctrl+Alt+Space` я
  показва и започва да слуша за глас. И двата се сменят в
  `keybindings.json` (`"global"`), а `""` ги изключва.
...
| ![ИИ чат](docs/screenshots/08-chat.jpg) | ![Светла тема](docs/screenshots/09-light.jpg) |
...
Последно пускане: **193 натискания, 43 клавишни комбинации, 74 открити команди, 0 грешки.** Освен че нищо не гърми, проверката държи и няколко неща, които трябва да са вярни: четири прозореца стават четири полета; станцията спира на 16 и го казва; полетата не се застъпват; прозорците стоят на цели пиксели и нищо не е мащабирано; F3 показва карта за всяка станция; в терминал `Ctrl+W` и `Ctrl+C` са на програмата; `Ctrl+K` работи и на българска подредба; ИИ навигацията срещу фалшива OpenAI-съвместима услуга — ключът не стига до платното, измислена команда не прави нищо, необратимото чака Enter. ИИ чатът говори с (фалшив) Genesis без чужд system prompt, а без Genesis минава на резервната услуга и казва защо; получава отговора на части и бутонът „В терминала“ написва командата без Enter. `Ctrl+Alt+L` сменя темата и тя се помни. Свой клавиш от `keybindings.json` работи веднага, старият се освобождава, сгрешеният се съобщава, а файл само с подразбиранията не променя нищо.
...
| `Ctrl+Shift+I` | ИИ чат |
| `Ctrl+Alt+L` | светла ↔ тъмна тема |
| `Ctrl+Alt+W` (отвсякъде в Windows) | покажи / скрий станцията |
| `Ctrl+Alt+Space` (отвсякъде в Windows) | покажи станцията и слушай |
| `Ctrl+Alt+N`, <code>Ctrl+`</code> | нов прозорец на Windows, обръщане между тях |

Всички клавиши се сменят в `keybindings.json`, в папката с настройките
(`%APPDATA%\Windows 20 Workstation`). Най-лесно е през **F1 → Промени
клавишите…**, което създава файла с всички клавиши:

```jsonc
{
  "keys": {
    // Прозорци: Нов терминал (в свободно поле)
    "window.terminal": ["Ctrl+T", "Ctrl+Shift+T"],
    // Прозорци: Към съседния прозорец (по ред: ляво, дясно, горе, долу)
    "window.focusDirection": ["Alt+H", "Alt+L", "Alt+K", "Alt+J"],
    // [] изключва клавиш
    "canvas.minimap": []
  },
  "global": { "show": "Ctrl+Alt+W", "voice": "Ctrl+Alt+Space" }
}
```
...
src/main/
  ...
  keybindings.js keybindings.json — свои клавиши и глобалните от Windows
  genesis.js     Genesis като мозък на чата: проверява и пуска API-то му
  ...
  ai.js          ИИ навигацията: изречение → една от командите;
                 чатът, с отговор на части и резервна верига
...
src/renderer/
  ...
  keymap.js      всички клавиши в една таблица, водещият клавиш, шпаргалката,
                 и свои клавиши от keybindings.json върху нея
  theme.js       тъмна / светла / като Windows
